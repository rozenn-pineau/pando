...
- testing the relationship between tree height and proportion of SNPs in the dataset to take into account missing snps: proportions (of snps kept) tested --> c(0.15,0.4,0.7,0.9,1)
...
run1 <- read.table("pando_2495snp_102inds.log", sep = "\t", header = T) 
run2 <- read.table("pando_3498snps_102inds.log", sep = "\t", header = T) 
run3 <- read.table("pando_3860snps_102inds.log", sep = "\t", header = T)
run4 <- read.table("pando_3934snps_102inds.log", sep = "\t", header = T)
run5 <- read.table("pando_3942snps_102inds.log", sep = "\t", header = T)
...
percent_snp_kept <- c(0.15,0.4,0.7,0.9,1)
...
x <- c(rep(percent_snp_kept[1], length(run1$Tree.height)),
       rep(percent_snp_kept[2], length(run2$Tree.height)),
       rep(percent_snp_kept[3], length(run3$Tree.height)),
       rep(percent_snp_kept[4], length(run4$Tree.height)),
       rep(percent_snp_kept[5], length(run5$Tree.height)))

y <- c(run1$Tree.height, 
       run2$Tree.height, 
       run3$Tree.height, 
       run4$Tree.height, 
       run5$Tree.height)
...
```{r plot}
xvals <- c(percent_snp_kept, 4)
yvals <- a*xvals + b

setwd("/Users/rozenn/GaTech Dropbox/CoS/BioSci/BioSci-Ratcliff/Rozenn/4.PandoProject/5-Writing/5.Figures/current/Figure6/")
pdf(file=paste("tree_height_missing_snps_relationship.pdf", sep="" ), bg = "transparent", width=5, height=3, family = "Times New Roman")

par(family="Times New Roman", cex.lab=1.2, cex.axis=1.2, cex.main=1, cex.sub=1)
plot(xvals, yvals, col = "#323232", type = "l", lty = 2, cex = 1.3, xlab = "Proportion of SNPs kept", ylab = "Tree Height", bty = "n", xlim = c(0, 1), ylim = c(0, 0.3))
...
SNPtot <- 3942 #number of mutations in this dataset
BPtot <- 129194577*0.5 #total number of base pair sequenced
N <- 3 #Ploidy

tree_height_bf_correction <- run5$Tree.height[run5$Tree.height< 0.5] # filter out tail of distribution
...
setwd("/Users/rozenn/GaTech Dropbox/CoS/BioSci/BioSci-Ratcliff/Rozenn/4.PandoProject/5-Writing/5.Figures/current/Figure6/")
pdf(file=paste("large_scale_pando_age.pdf", sep="" ), bg = "transparent", width=5.5, height=3, family = "Times New Roman")
...
setwd("/Users/rozenn/GaTech Dropbox/CoS/BioSci/BioSci-Ratcliff/Rozenn/4.PandoProject/17-analyses/4_age/5_results/pando/dec28/2495")
bsp <- fread("pando_2495snp_102inds-pando_2495snp_102inds_skyline.txt")
...
setwd("/Users/rozenn/GaTech Dropbox/CoS/BioSci/BioSci-Ratcliff/Rozenn/4.PandoProject/5-Writing/5.Figures/current/Figure6/")
pdf(file=paste("BSP_large_scale.pdf", sep="" ), bg = "transparent", width=5, height=3, family = "Times New Roman")

par(family="Times New Roman", cex.lab=1.2, cex.axis=1.2, cex.main=1, cex.sub=1)
plot(bsp$age, log(bsp$median), type = "l" , bty = "n", xlim = c(25000, 0), ylim = c(min(log(bsp$mean)), max(log(bsp$mean))),
     xlab = "Age (years)", ylab = "Effective population size (log)")
# model before correction(
polygon(c(rev(bsp$age), bsp$age), c(log(rev(bsp$lower)), log(bsp$upper)), col = '#ccece6', border = NA) # add fill
...
setwd("/Users/rozenn/GaTech Dropbox/CoS/BioSci/BioSci-Ratcliff/Rozenn/4.PandoProject/17-analyses/4_age/5_results/pando/dec28/2495/")
tree <- read.tree("pando_2495snp_102inds-pando_2495snp_102inds.newick") # to be saved in Newick format
